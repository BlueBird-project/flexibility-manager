run_EF_assessment_predict <- function(buildingSubject, df_predict, settings, libraryPath=".", modelMode="predict", plots=F){
  ...
  write("",stderr())
  write("########################################################",stderr())
  write("####### Flexibilty Measures Assessment Predictor #######",stderr())
  write("########################################################",stderr())
  ...
  if (nhorisons < 1) {
    write("There are not enough lags information, the input should include at least 5 lags before the first sample that must be predicted")
    return(NULL)
  }
  ...
  if (modelMode=="predict") {
    ...
    # Obtain start and end variables for the results dataframe and convert them in ISO 8601 format
    df_result$start <- df_result$time
    
    df_result <- df_result %>%
      mutate(end = start + hours(1))
    
    df_result$start <- sub("(\\+|\\-)(\\d{2})(\\d{2})", "\\1\\2:\\3",
                                        format(lubridate::ymd_hms(df_result$start, tz = "UTC"), "%Y-%m-%dT%H:%M:%S%z")
    )
    df_result$end <- sub("(\\+|\\-)(\\d{2})(\\d{2})", "\\1\\2:\\3",
                                      format(df_result$end, "%Y-%m-%dT%H:%M:%S%z")
    )
    df_result <- df_result[, !names(df_result) %in% "time"]
