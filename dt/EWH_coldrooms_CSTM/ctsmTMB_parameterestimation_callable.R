...
{
  FridgeModel <- ctsmTMB$new()
  
  FridgeModel$addInput(u1, u2, u3, u4,
                 d1, d2, d3, d4)
...
{ 
  ...
  FridgeModel$setInitialState(list(x0 = x0, p0 = 1e-4 * diag(8)))
  
}

{

  FreezerModel <- ctsmTMB$new()
  
  FreezerModel$addInput(u5=u5, d5=d5)
  ...
  if (estimate_Q_door_peak) {
    ...
    FreezerModel$addSystem(
      # Room 5
      dTa5 ~ ( k1_5*(Tamb - Ta5) + k2_5*(Tp5 - Ta5) - k3_5*u5 + k4_5*d5*1000 ) * dt + exp(sigma_a) * dw9,
      dTp5 ~ ( k5_5*(Ta5 - Tp5) ) * dt + exp(sigma_a) * dw10
    )
    ...
    FreezerModel$addSystem(
      # Room 5
      dTa5 ~ ( k1_5*(Tamb - Ta5) + k2_5*(Tp5 - Ta5) - k3_5*u5 + k4_5*d5 ) * dt + exp(sigma_a) * dw9,
      dTp5 ~ ( k5_5*(Ta5 - Tp5) ) * dt + exp(sigma_a) * dw10
    )
  }
}
  # ── Observation equation ─────────────────────────────────────────────────────
  FreezerModel$addObs(y_Ta5 ~ Ta5)
{ 
  FreezerModel$setVariance(y_Ta5 ~ sigma_obs^2)
  
  # ── Parameters ──────────────────────────────────────────────────────────────
  FreezerModel$setParameter(
    ...
    # Shared COP parameter (assuming same cooling system)
    k3_5 = c(initial = 0.01, lower = 0, upper = 10),
  ...
  FreezerModel$setInitialState(list(x0 = x0, p0 = 1e-4 * diag(2)))
...
fridge_fit <- FridgeModel$estimate(
  df.train,
  use.hessian = TRUE,
  method  = "ekf",
  control = list(eval.max = 100000, iter.max = 4000, trace = 1)
)

freezer_fit <- FreezerModel$estimate(
  df.train,
  use.hessian = TRUE,
  method  = "ekf",
  control = list(eval.max = 100000, iter.max = 4000, trace = 1)
...
all_params <- rbind( FridgeModel$getParameters(), FreezerModel$getParameters() )
...
ss_model <- build_discrete_state_space_multiroom_v2(
  ...
  n_rooms = 5,
  time_unit = "seconds",
  estiamte_Q_door_peak = estimate_Q_door_peak
)
